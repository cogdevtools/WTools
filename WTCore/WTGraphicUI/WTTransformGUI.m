...
classdef WTTransformGUI
    ...
    methods(Static, Access = private)
        ...
        function fig = plotFWHM(isNormalised, sampRate, freqMin, freqMax, freqRes, timeRes, wvtCycles)
            freqs = (freqMin : freqRes : freqMax);
        
            FWHMt = WTProcessUtils.morletKernelFWHM(sampRate, ...
                    timeRes, ...
                    isNormalised, ...
                    freqs, ...
                    wvtCycles);
            ...
            nam = 'WAVELETS FULL WIDTH AT HALF MAXIMUM';
            tit = sprintf('Frequency vs FWHM/2, cycles = %d', wvtCycles);
            fig = figure('Name', nam,  'NumberTitle', 'off');
            plot(freqs, FWHMt_half, '*:', 'LineWidth', 2, 'MarkerSize', 6);
            xlabel('Frequency (Hz)', 'FontSize', 12);
            ylabel('FWHM/2 (seconds)', 'FontSize', 12);
            title(tit, 12);
            grid on;
            for i = 1:length(freqs)
                text(freqs(i), FWHMt_half(i), sprintf('%.4f', FWHMt_half(i)), ...
    ...
    methods(Static)
        function success = defineCWTParams(waveletTransformPrms, samplingRate, timeRange, maxFreq, maxChans)
            success = false;
            ...
            answer = { ...
                ...
                waveletTransformPrms.TransformComponents, ...
                waveletTransformPrms.TransformPower, ...
                waveletTransformPrms.WaveletsCycles, ...
            };
        
            FWHMCbData = WTHandle(struct('plotFun', @plotFWHM, 'fig', []));
            FWHMCb = 'cbData = get(gcbo, ''userdata''); cbData.Value.plotFun(cbData);';

            function closeFWHMPlot(cbData)
                if ~isempty(cbData.Value.fig) && isvalid(cbData.Value.fig)
                    close(cbData.Value.fig)
                end
            end

            function plotFWHM(cbData) 
                try 
                     closeFWHMPlot(cbData);
                     cbData.Value.fig = WTTransformGUI.plotFWHM( ...
                         get(findobj(gcbf, 'tag', 'PW'), 'value'), ...
                         samplingRate, ... 
                         WTNumUtils.str2double(get(findobj(gcbf, 'tag', 'FL'), 'string')), ...
                         WTNumUtils.str2double(get(findobj(gcbf, 'tag', 'FH'), 'string')), ...
                         WTNumUtils.str2double(get(findobj(gcbf, 'tag', 'FR'), 'string')), ...
                         WTNumUtils.str2double(get(findobj(gcbf, 'tag', 'TR'), 'string')), ...
                         WTNumUtils.str2double(get(findobj(gcbf, 'tag', 'WC'), 'string'))); ...
                catch me  
                      wtLog = WTLog(); 
                      wtLog.except(me); 
                      wtLog.err('Failed to display FWHM plot: check the transform params...'); 
                end
            end
            ...
            function params = setParameters(answer) 
                params = { ...
                    ...
                    { 'style' 'edit'       'string' answer{1,3} 'tag' 'TR' } ...
                    { 'style' 'text'       'string' sprintf('Frequency (Hz) [%d,%d]: From', 1, maxFreq) } ...
                    { 'style' 'edit'       'string' answer{1,4} 'tag' 'FL' } ...
                    { 'style' 'text'       'string' 'To' } ...
                    { 'style' 'edit'       'string' answer{1,5} 'tag' 'FH' } ...
                    { 'style' 'text'       'string' 'By (default = 1)' } ...
                    { 'style' 'edit'       'string' answer{1,6} 'tag' 'FR' } ...
                    ...
                    { 'style' 'checkbox'   'value'  answer{1,10} } ...
                    { 'style' 'text'       'string' '' } ...
                    { 'style' 'text'       'string' '' } ...
                    { 'style' 'text'       'string' '' } ...
                    { 'style' 'text'       'string' '' } ...
                    { 'style' 'text'       'string' 'Save Re/Im components' } ...
                    { 'style' 'checkbox'   'value'  answer{1,11} } ...
                    { 'style' 'text'       'string' '' } ...
                    { 'style' 'text'       'string' '' } ...
                    { 'style' 'text'       'string' '' } ...
                    { 'Style' 'pushbutton' 'string' 'FWHM / 2' 'callback' FWHMCb 'userdata' FWHMCbData } ...
                    { 'style' 'text'       'string' 'Transform Power' } ...
                    { 'style' 'checkbox'   'value'  answer{1,12} 'tag' 'PW' } ...
                    { 'style' 'text'       'string' '' } ...
                    { 'style' 'text'       'string' '' } ...
                    { 'style' 'text'       'string' 'Wavelet cycles: value in [2,15])' } ...
                    { 'style' 'edit'       'string' answer{1,13} 'tag' 'WC'  }};
            end
            ...
            waveletTransformPrms.TransformComponents = reIm;
            waveletTransformPrms.TransformPower = transformPower;
