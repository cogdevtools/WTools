...
function wtStatistics(subjectsList, conditionsList, channelsList, evokedOscillations)
    ...
    if interactive
        [channelsList, ~] = WTUtils.stringsSelectDlg('Select channels\nfor statistics:', allChannelsLabels, false, true);
    elseif isempty(channelsList)
        channelsList = allChannelsLabels;
    else 
        intersectChannelsList = sort(intersect(channelsList, allChannelsLabels));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function datatab=WTStatistics_(ChannelsList,tMin,tMax,FrMin,FrMax,indFr,varargin)
    ...
    if ~nargin
        ...
        if ~exist('PROJECTPATH','var')
            ...
            if exist('subjgrand.m','file') && exist('condgrand.m','file')
                ...
                % Ask the user whether new subjectsList have been added and processed to the
                % already existing project
                parameters = { ...
                    { 'style' 'text' 'string' 'Have you added any new subjectsList to the project?' } ...
                ...
                if strcmp(strhalt,'retuninginputui')
                    if exist('subjectsList','var') % Necessary to make subjectsList working in the function workspace
                        subjN = size(subjectsList,2);
                    else
                ...
                assignin('caller','subjectsList',subjectsList);
                fprintf(2,'\nFile-list of transformed subjectsList and/or conditions not found!!!\n');
                fprintf('\n');
                return
            end        
        end
        ...
        assignin('base','subjectsList',subjectsList);
        ...
        try
            firstSubj = strcat (CommonPath,subjectsList(1),'/',subjectsList(1),'_',conditions(1),'_bc-avWT.mat');
            load (char(firstSubj));
        catch
            firstSubj = strcat (CommonPath,subjectsList(1),'/',subjectsList(1),'_',conditions(1),'_bc-evWT.mat');
            load (char(firstSubj));
        end
        assignin('base','chanlocs',chanlocs);
        
        cb_subj = [ ...
            'if length(subjectsList)>1,' ...
            '[subjlist, ok] = listdlg(''PromptString'',''Select subjectsList to retrieve:'',''SelectionMode'',''multiple'',''ListString'',subjectsList);' ...
            'if ~ok,' ...
            'assignin(''caller'',''subjectsList'',subjectsList);' ...
            'return;' ...
            'end;' ...
            'subjectsList = subjectsList(subjlist);' ...
            'else;' ...
            'subjectsList = subjectsList(1);' ...
        ...
        if isempty(answer)
            assignin('caller','subjectsList',subjectsList);
            evalin('base','clear chanlocs chans conditions condlist defaultanswer labels ok subjectsList subjlist');
            return % quit on cancel button
        end
        
        % import variables from the base workspace
        subjectsList = evalin('base','subjectsList');
    ...
    elseif ischar(varargin) && strcmp(varargin,'evok')    
        ...
        assignin('caller','subjectsList',subjectsList);
        wtLog.warn(['The measure % s is not present in the subjectsList folders! If you want to retrieve averages from\n'...
    ...
    if ~indFr % for frequency band...
        datatab=cell(subjN+1,condN*channelsN+1); % 1 row and 1 colon extra to write subjectsList and conditions
        datatab{1,1}='subj';
    else % ... or for indivudual frequencies
        FrN=length(f1:f2);
        datatab=cell(subjN+1,condN*channelsN+1,FrN); % 1 row and 1 colon extra to write subjectsList and conditions
        datatab(1,1,:)={'subj'};
    end

    % WRITE subjectsList list in the matrix
    for s = 1:subjN
        if ~indFr
            datatab{s+1,1}=char(strcat('#',subjectsList(s)));
        else
            datatab(s+1,1,:)={char(strcat('#',subjectsList(s)))};
        end
    end
    ...
    for s = 1:subjN    
        ...
        for cn = 1:condN       
            currectSubj = fullfile(strcat(CommonPath,subjectsList(s)), strcat(subjectsList(s),'_',conditions(cn),measure));        
    ...
    % Assign the variable subjectsList to the caller workspace
    assignin('caller','subjectsList',subjectsList);
