function tf_cmor(subjects,tmin,tmax,tres,frmin,frmax,fres,extraedges,ChannelsList,EpochsList,normalizeWavelet,cycles,logtransform,varargin)
    ...
    if ~WTProject.isRootDirSet()
        WTLog.warn('No open project')
        return
    end
    [success, filename] = WTProject.readFilePrefix('filename');
    if ~success 
        WTUtils.msggui('Error', 'The project looks corrupted: check the log...');
        return
    end
    [success, subjects] = WTProject.readSubjects('subjects');
    if ~success 
        WTUtils.msggui('Error', 'Theh subjects file is missing!\nDid you copy/import the data from EEGLAB?');
        return
    end
    [success, conditions] = WTProject.readConditions('conditions');
    if ~success 
        WTUtils.msggui('Error', 'The conditions file is missing!\nTry to import again from EEGLAB...');
        return
    end

    PROJECTPATH = WTProject.getRootDir();
    ...
    if nargin
        ...
        elseif ~iscell(subjects)
            WTLog.err(['Please, enter a subject number in the right format (e.g. tf_cmor(''01'',...);) to process\n'...
                'an individual subject, or edit ''subj.m'' in the ''cfg'' folder and enter empty value,\n'...
                '(i.e. tf_cmor([],...);), to process the whole sample.']);
            return
        end
    end
    ...
    if ~nargin
        ...
        pop_cfgfile = fullfile(PROJECTPATH,'pop_cfg','subjgrand.m');
        ...
        pop_cfgfile = fullfile(PROJECTPATH,'pop_cfg','condgrand.m');
        fid = fopen(pop_cfgfile, 'wt'); %Overwrite preexisting file with the same name
        ...
        [enable_uV logFlag last_tfcmor last_bschop bs_Log]=check_evok_log(PROJECTPATH);
        enable_Log='on';
        if bs_Log
            enable_Log='off';
        end
        ...
        pop_cfgfile = fullfile(PROJECTPATH,'pop_cfg','tf_cmor_cfg.m');
        if exist(pop_cfgfile,'file')
            ...
            try
                ...
                WTLog.err(['The tf_cmor_cfg.m file in the pop_cfg folder was created by a previous version\n'...
                    'of WTools. It will be updated to the most recent version and overwritten.']);
        ...
        [log, EEG] = evalc('pop_loadset(''filename'', currectSubj, ''filepath'', InPath)');
        ...
        while true
            ...
            answer = WTUtils.inputgui( 'geometry', geometry, 'uilist', parameters,'title', 'Set wavelet transformation parameters');
    ...
    for i = 1:subjN
        for j = 1:condN
            ...
            try
                [ALLEEG, EEG, CURRENTSET, ALLCOM] = WTLog.evalcLog(WTLog.LevelInf, 'EEGLAB', 'eeglab');
            catch ME
                WTLog.err(ME.message);
                WTLog.err('EEGLAB is not running: please, start it!');
                WTLog.ctxOff();
                return
            end
            
            EEG = WTLog.evalcLog(WTLog.LevelInf, 'EEGLAB', 'pop_loadset(''filename'', currectSubj, ''filepath'', InPath)');
            [ALLEEG, EEG, CURRENTSET] = WTLog.evalcLog(WTLog.LevelInf, 'EEGLAB', 'eeg_store(ALLEEG, EEG, 0)');
            EEG =  WTLog.evalcLog(WTLog.LevelInf, 'EEGLAB', 'eeg_checkset(EEG)');
            ...
            if ~isempty(varargin)
                ...
                elseif ~strcmp(varargin, 'evok')
                    WTLog.err(['\nThe measure %s is unknown.\n'...
                        'If you want to compute evoked oscillations, add ''evok'' as last argument.\n'...
                        'Do not add it if you want to compute total-induced oscillations.'], varargin);
                    WTLog.ctxOff()
                    return
                end
