...
if ~nargin
    ...
    if ~exist('PROJECTPATH','var')
        ...
        if exist('subjgrand.m','file') && exist('condgrand.m','file')
            ...
            geometry = { [1] [1] };
            [answer userdat strhalt] = inputgui('geometry',geometry,'uilist',parameters,'title','Subjects lists rebuild');
    ...
    answer = inputgui( 'geometry', geometry, 'uilist', parameters,'title', 'Set average retrieving parameters');
...
elseif ~strcmp(varargin,'evok')    
    ...
    fprintf(2,'If you want to retrieve averages from evoked oscillations, please type ''evok'' as last\n');
    fprintf(2,'argument (after the FrMax argument).\n');
    fprintf(2,'Type nothing after the FrMax argument if you want to retrieve averages from total-induced oscillations.\n');
    fprintf('\n');
    return    
end
    ...
    for ch = 1:channelsN        
        ...
        if ~nargin            
            ...
            if isempty(chan)
                assignin('caller','subjects',subjects);
                fprintf(2,'\nChannel %s not found!!!\n', char(actualchan));
                fprintf(2,'The channel you are trying to retrieve might had not been transformed.\n');
...
for s = 1:subjN    
    ...
    for cn = 1:condN       
        currectSubj = strcat (CommonPath,subjects(s),'/',subjects(s),'_',conditions(cn),measure);        
...
%... inform the user
fprintf('\n');
fprintf('File %s.tab successfully saved in the stat folder!!! \n',strcat(OutFileName,measure(1:findstr(measure,'.mat')-1)));
fprintf('\n');
