...
if exist('PROJECTPATH','var')
    ...
    addpath(strcat(PROJECTPATH,'/Config'));
    pop_cfgfile = strcat(CommonPath,'Config/smavr3d_cfg.m');
else
    CommonPath = strcat ('../');
    alreadyexistdir=strcat(CommonPath,'Config');
    if ~exist(alreadyexistdir,'dir')
    ...
    pop_cfgfile = strcat('../Config/smavr3d_cfg.m');
end
...
if ~nargin
    ...
    if exist(pop_cfgfile,'file')
        ...
        try
            ...
            fprintf('\n');
            fprintf(2, 'The smavr3d_cfg.m file in the Config folder was created by a previous version\n');
