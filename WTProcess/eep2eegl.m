function eep2eegl(subjects,epochlimits,hpf,lpf)
...
%Usage:
%
%eep2eegl(subjects,epochlimits,hpf,lpf)
%eep2eegl('02',[-200 1000],0.3,65)
%eep2eegl([],[-200 1000],0.3,65)
...
if ~nargin    
    if ~exist('PROJECTPATH','var')
        ...
        for i=1:length(filenames)
            subtoimport(i) = str2num(filenames{1,i}(findstr(filenames{1,i},'_')+1:findstr(filenames{1,i},'.')-1));
        end
...
elseif ~iscell(subjects)
    fprintf(2,'\nPlease, enter a subject number in the right format (e.g. eep2eegl(''01'');) to process\n');
    fprintf(2,'an individual subject, or edit ''subj.m'' in the ''cfg'' folder and enter nothing,\n');
    fprintf(2,'(i.e. eep2eegl();), to process the whole sample.\n');
    fprintf('\n');
    return
end
...
for i = 1:subjN
    ...
    if exist('PROJECTPATH','var') && i==1 %do it only once        
        ...
        if length(allfields)>1            
            ...
            for k=1:length(allfields)
                allfields{1,k}=num2str(str2num(allfields{1,k}));
            end            
        end
    ...
    if ~nargin && i==1
        ...
        pop_cfgfile = strcat(PROJECTPATH,sla,'pop_cfg',sla,'eep2eegl_cfg.m');
        if exist(pop_cfgfile,'file')
            ...
            defaultanswer{1,answersN};
                ...
                fprintf('\n');
                fprintf(2, 'The eep2eegl_cfg.m file in the pop_cfg folder was created by a previous version\n');
